...
export function parseGithubRepositoryUrl(value) {
  ...
  if (sshMatch) {
    return buildParsedGithubRepository(sshMatch[1], sshMatch[2]);
  }
...
function buildParsedGithubRepository(owner, repoName) {
  ...
  if (!normalizedOwner || !normalizedRepoName) {
    return null;
  }
...
function resolveSandboxNetworkMode(value) {
  ...
  if (value === 'allow_all' || value === 'allow_list') {
    return value;
  }
...
export function resolveAgentSandboxTemplatePatchFromEnv() {
  ...
  const repoUrl = (process.env.AGENT_SANDBOX_GITHUB_REPO_URL ?? '').trim();
  const branch = (process.env.AGENT_SANDBOX_GITHUB_REPO_BRANCH ?? '').trim() || DEFAULT_BRANCH;
  const pat = (process.env.AGENT_SANDBOX_GITHUB_PAT ?? '').trim();

  if (!repoUrl) {
    if (pat) {
      throw new Error('AGENT_SANDBOX_GITHUB_PAT requires AGENT_SANDBOX_GITHUB_REPO_URL.');
    }
  ...
  if (!BRANCH_PATTERN.test(branch)) {
    throw new Error('AGENT_SANDBOX_GITHUB_REPO_BRANCH must be a non-empty branch name without spaces.');
  }
  ...
  const sandboxNetworkMode = resolveSandboxNetworkMode(getOptionalEnv('AGENT_SANDBOX_NETWORK_MODE'));
  const sandboxAllowedOutboundHosts = normalizeSandboxAllowedOutboundHosts(
  ...
  return {
    configPatch: {
      ...
      sandbox_allowed_outbound_hosts: sandboxAllowedOutboundHosts,
